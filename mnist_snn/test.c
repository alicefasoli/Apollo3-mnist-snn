...
int main()
{
    int i, j, k, m, n, x, y, tm, t1, n_mnist;
    ...
    for (n_mnist = 0; n_mnist < max; n_mnist++)
    {
        start = clock();
        printf("[%d]: \n", n_mnist);
        for (i = 0; i < PIXEL; i++) // loop for pixel in axes x
        {
            ...
            for (j = 0; j < PIXEL; j++) // loop for pixel in axes y
            {
                if (mode == 0)
                {
                    actual_img[i][j] = train_image[n_mnist][(i * PIXEL) + j];
                }
                else
                {
                    actual_img[i][j] = test_image[n_mnist][(i * PIXEL) + j];
                }
            ...
            for (i = 0; i < N_SECOND_LAYER; i++)
            {
                if (output_layer[i]->t_rest < tm)
                {
                    ...
                    if (output_layer[i]->p > p_rest)
                    {
                        ...
                        if (output_layer[i]->p_th > p_th && mode == 0)
                        {
        ...
        if (mode == 0)
        {
            ...
            neuron_labels[winner] = train_label[n_mnist];
        ...
        else
        {
            ...
            if (neuron_labels[learning_neuron] == test_label[n_mnist])
            {
    ...
    if (mode == 0)
    {
        for (i = 0; i < N_SECOND_LAYER; i++)
        {
            fprintf(f_labels, "%d,", neuron_labels[i]);
            for (j = 0; j < N_FIRST_LAYER; j++)
            {
